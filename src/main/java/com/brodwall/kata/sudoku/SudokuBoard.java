...
public class SudokuBoard {
    ...
    public List<Integer> getOptionsForCell(int row, int column) {
        ...
        removeOptionsOnRow(options, row);
        removeOptionsOnColumn(options, column);
    ...
    private void removeOptionsOnColumn(List<Integer> options, int column) {
        for (int row=0; row<SIZE; row++) options.remove(board[row][column]);
    }

    private void removeOptionsOnRow(List<Integer> options, int row) {
    ...
    public String dumpBoard() {
        ...
        for (Integer[] row : board) {
            for (Integer cell : row) {
                boardAsString.append(cell != null ? cell.toString() : ".");
            }
    ...
    public void readBoard(String boardAsString) {
        ...
        for (Integer[] row : board) {
            for (int column = 0; column < row.length; column++) {
                row[column] = cellValue(boardAsString.charAt(index++));
            }
        }
    }

    private Integer cellValue(char c) {
